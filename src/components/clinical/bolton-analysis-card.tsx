...
export function BoltonAnalysisCard({
  ...
  return (
    <Card className="border-slate-200 shadow-sm bg-white overflow-hidden" dir={isAr ? 'rtl' : 'ltr'}>
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-[11px] text-slate-500">{t.boltonSubtitle}</p>
            </div>
          </div>
      ...
      <CardContent className="p-5 space-y-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          ...
          <div className="p-3.5 rounded-xl border border-slate-200 bg-slate-50/50 space-y-2">
            ...
            <div className="text-[11px] text-slate-600 flex justify-between">
              <span>{isAr ? 'المعيار الطبيعي:' : 'Norm:'} <strong>77.2% ± 1.65%</strong></span>
              <span>{isAr ? 'الفارق:' : 'Diff:'} <strong>{Number(anteriorDiscrepancy) > 0 ? `+${anteriorDiscrepancy}` : anteriorDiscrepancy}%</strong></span>
            </div>
            ...
            <p className="text-[10px] text-slate-500 italic">
              {isAr ? (
                Number(anteriorDiscrepancy) > 1.5 
                  ? 'فائض في حجم قواطع الفك السفلي: يوصى ببرد خفيف بين الأسنان (IPR) أو بناء الأسنان العلوية.'
                  : Number(anteriorDiscrepancy) < -1.5
                  ? 'فائض في حجم قواطع الفك العلوي: يوصى ببرد IPR للفك العلوي أثناء مراحل الإنهاء.'
                  : 'تناسق نموذجي مثالي بين الأنياب العلوية والسفلية (Bolton Harmonious).'
              ) : (
                Number(anteriorDiscrepancy) > 1.5 
          ...
          <div className="p-3.5 rounded-xl border border-slate-200 bg-slate-50/50 space-y-2">
            ...
            <div className="text-[11px] text-slate-600 flex justify-between">
              <span>{isAr ? 'المعيار الطبيعي:' : 'Norm:'} <strong>91.3% ± 1.91%</strong></span>
              <span>{isAr ? 'الفارق:' : 'Diff:'} <strong>{Number(overallDiscrepancy) > 0 ? `+${overallDiscrepancy}` : overallDiscrepancy}%</strong></span>
            </div>
            ...
            <p className="text-[10px] text-slate-500 italic">
              {isAr ? (
                Math.abs(Number(overallDiscrepancy)) <= 1.9 
                  ? 'نسبة الـ 12 سناً لكامل القوسين ضمن الحدود الطبيعية. إطباق صنف أول متداخل ومثالي قابل للتحقيق.'
                  : 'فارق طفيف في الأسنان الخلفية. يتم تعويضه عبر ثنيات السلك من الدرجة الثانية (Second-Order).'
              ) : (
                Math.abs(Number(overallDiscrepancy)) <= 1.9 
        ...
        <div className="border border-slate-200 rounded-xl p-4 bg-white space-y-3">
          <div className="flex items-center justify-between">
            ...
            <span className="text-[10px] text-slate-400 font-medium">{isAr ? 'طريقة كاري / نانس' : 'Carey / Nance Method'}</span>
          </div>

          <div className="grid grid-cols-2 gap-4 text-xs">
            <div className="p-2.5 rounded-lg bg-slate-50 border border-slate-100 space-y-1">
              <div className="text-[11px] text-slate-500 font-medium">{t.upperArchSpace}:</div>
              <div className="text-base font-black text-slate-900 flex items-center gap-2">
                <span className={upperSpaceDeficit < -4 ? 'text-rose-600' : upperSpaceDeficit < 0 ? 'text-amber-600' : 'text-emerald-600'}>
                  {upperSpaceDeficit === 0 ? (isAr ? 'متزن (0 مم)' : 'Balanced (0 mm)') : `${upperSpaceDeficit} ${isAr ? 'مم' : 'mm'}`}
                </span>
              ...
              <p className="text-[10px] text-slate-500">
                {isAr ? (
                  upperSpaceDeficit <= -6 
                    ? '⚠️ عجز شديد في المساحة. يوصى بخلع ضواحك علاجياً أو توسيع عظمي هيكلي RPE.' 
                    : upperSpaceDeficit < 0 
                    ? 'تزاحم علوي خفيف إلى متوسط. يمكن حله بتوسيع تقويمي مع برد خفيف IPR.' 
                    : 'المساحة المحيطية كافية لرص الأسنان بالكامل.'
                ) : (
                  upperSpaceDeficit <= -6 
            ...
            <div className="p-2.5 rounded-lg bg-slate-50 border border-slate-100 space-y-1">
              <div className="text-[11px] text-slate-500 font-medium">{t.lowerArchSpace}:</div>
              <div className="text-base font-black text-slate-900 flex items-center gap-2">
                <span className={lowerSpaceDeficit < -4 ? 'text-rose-600' : lowerSpaceDeficit < 0 ? 'text-amber-600' : 'text-emerald-600'}>
                  {lowerSpaceDeficit === 0 ? (isAr ? 'متزن (0 مم)' : 'Balanced (0 mm)') : `${lowerSpaceDeficit} ${isAr ? 'مم' : 'mm'}`}
                </span>
                <span className="text-[10px] text-slate-400 font-normal">
                  ({crowdingLower})
                </span>
              ...
              <p className="text-[10px] text-slate-500">
                {isAr ? (
                  lowerSpaceDeficit <= -5 
                    ? '⚠️ عجز حرج في مساحة الفك السفلي. يجب الحفاظ على القواطع السفلية داخل العظم السنخي وتجنب إمالتها للخارج.' 
                    : lowerSpaceDeficit < 0 
                    ? 'تزاحم سفلي متوسط. يتم حل المساحة ببرد بيني IPR أو تحسين شكل القوس.' 
                    : 'محيط الفك السفلي ملائم ومستقر.'
                ) : (
                  lowerSpaceDeficit <= -5 
